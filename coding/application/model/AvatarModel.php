...
class AvatarModel
{
     ...
     * The URL (or image) always returns a .jpg file ! For deeper info on the different parameter possibilities:
    ...
    public static function getPublicUserAvatarFilePathByUserId($user_id)
    {
        ...
        $query = $database->prepare("SELECT has_local_avatar
                                     FROM user
                                     WHERE id = :user_id
                                     LIMIT 1");
        $query->execute([':user_id' => $user_id]);

        if ($query->fetch()->has_local_avatar) {
            return Config::get('URL') . Config::get('PATH_AVATARS_PUBLIC') . $user_id . '.jpg';
        }
    ...
    /**
     * Creates an avatar picture (and checks all necessary things too).
     * TODO decouple
     * TODO total rebuild
     */
    ...
    public static function validateImageFile()
    {
        ...
        if (!File::checkFileSize($_FILES['avatar_file'])){
            Session::add('feedback_negative', Text::get('FEEDBACK_AVATAR_UPLOAD_TOO_BIG'));
            return false;
        }
