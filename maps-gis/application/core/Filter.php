...
class Filter
{
     ...
     *
     * A very simple introduction: Let's say an attacker changes its username from "John" to these lines:
     * "<script>var http = new XMLHttpRequest(); http.open('POST', 'example.com/my_account/delete.php', true);</script>"
     * This means, every user's browser wouldn't render "John" anymore, instead interpreting this JavaScript code, calling
     ...
     * harmless HTML (you'll see the code, it will not be interpreted). Then the method checks if the value is an array, 
     * or an object and if so, makes sure all its string content is encoded (recursive call on its values).
     * Note that this method uses reference to the passed variable, not a copy, meaning you can use this methods like this:
     *
