...
class View
{
     ...
     */
    public function render($fileName, $data = null, $customHeader = '')
    {
        $this->loadData($data);

        $this->loadHeadersAndNavigation($fileName, $customHeader);
     ...
     * @return bool
     */
    public function renderMulti($fileNames, $data = null, $customHeader = '')
    {
        if (!is_array($fileNames)) {
            self::render($fileNames, $data, $customHeader);
            return false;
        }

        $this->loadData($data);

        $this->loadHeadersAndNavigation($fileNames[0], $customHeader);
    ...
    protected function loadHeadersAndNavigation($fileName, $customHeader = ''){
        $data['navItemsStream'] = '';
        $data['navLoginStream'] = '';
        ...
        // Load view
        $this->loadHeaderStart();
        $this->loadHeaderCustom($customHeader);
        $this->loadHeaderEnd();
        $this->loadNavigation();
        $this->loadSidebar();
    }

    protected function loadHeaderStart(){
        require Config::get('PATH_VIEW') . '_templates/headerStart.php';
    }

    protected function loadHeaderCustom($customHeader){
        if (!empty($customHeader)){
            require Config::get('PATH_VIEW') . $customHeader;
        }
    }

    protected function loadHeaderEnd(){
        require Config::get('PATH_VIEW') . '_templates/headerEnd.php';
    }
