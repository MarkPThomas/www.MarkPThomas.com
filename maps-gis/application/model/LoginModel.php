...
class LoginModel
{
    ...
    public static function login($user_name, $user_password, $set_remember_me_cookie = null)
    {
        ...
        self::setSuccessfulLoginIntoSession(
            $result->id, $result->username, $result->email, $result->user_account_type_id
        );
    ...
    public static function loginWithCookie($cookie)
    {
        ...
        if ($result) {
            ...
            // successfully logged in, so we write all necessary data into the session and set "user_logged_in" to true
            self::setSuccessfulLoginIntoSession($result->id, $result->username, $result->email, $result->user_account_type_id);
    ...
    public static function resetFailedLoginCounterOfUser($user_name)
    {
        ...
        $sql = "UPDATE user
                 ...
                 WHERE username = :user_name AND failed_logins != 0
                 LIMIT 1";
        $sth = $database->prepare($sql);
    ...
    public static function saveTimestampOfLoginOfUser($user_name)
    {
        ...
        $sql = "UPDATE user
                SET last_login_timestamp = :user_last_login_timestamp
    ...
    public static function setRememberMeInDatabaseAndCookie($user_id)
    {
        ...
        $sql = "UPDATE user
                SET remember_me_token = :user_remember_me_token
                WHERE id = :user_id
                LIMIT 1";
        $sth = $database->prepare($sql);
    ...
    {
        // if $user_id was set, then clear remember_me token in database
        if (isset($user_id)) {
